...
const EvalIndicatorGuyeh: React.FC = () => {
  ...
  const [fields, setFields] = useState<Field[]>([{ label: '', number: '' }]);
  ...
  const handleAddApi = async () => {
    ...
    if (title !== "" && fields) {
      await addEvalItem(title, location.state, fields);
      setaddModal(false);
    } else {
      setEmptyError(true);
      setTimeout(() => {
  ...
  useEffect(() => {
    if (dataIndicatorGuyeh) {
      setTitle(dataIndicatorGuyeh[0]?.title)
      setData(dataIndicatorGuyeh[0]?.data)
    }
