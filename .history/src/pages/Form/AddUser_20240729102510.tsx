...
const AddUser: React.FC = () => {
  ...
  useEffect(() => {
    if (error || errorAdd ||  errorOL, errorOU) {
  ...
  }, [error, errorAdd, errorOL, errorOU]);

  return (
    <>

      {showErrorTimer && showErrorType && <Error myError={"لطفا فیلد ها را به درستی وارد کنید!"} />}
      {showErrorTimer && showError && <Error myError={"لطفا تمام فیلد ها را پر کنید!"} />}
      {showErrorTimer && errorAdd && <Error myError={"عملیات موفقیت آمیز نبود!"} />}

      {showErrorTimer && doneAddAdminUser && (
        <div
