...
import AssignmentSupervieorDelete from '../../hooks/AssignmentSupervisorIndicatorStore/';
...
const AssignmentSupervisorIndicator: React.FC = () => {
  ...
  const handleDeleteItem = async (id: number) => {
    await getAdminSADeletebasedPeriodSupervisorEmployee(selectedSupervisor, selectedPeriod, id);
  };

  const handleDeleteBasedSelected = async () => {
    if (selectedPeriod && employementList.length == 0 && !selectedSupervisor) {
      await getAdminSADeletebasedPeriod(selectedPeriod);
    } else if (selectedPeriod && selectedSupervisor && employementList.length == 0) {
      await getAdminSADeletebasedPeriodSupervisor(selectedSupervisor, selectedPeriod);
    }
  };
