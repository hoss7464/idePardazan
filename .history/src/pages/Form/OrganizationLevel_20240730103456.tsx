...
const OrganizationLevel: React.FC = () => {
  ...
  const handleupdateOL = async (title: string, id: string) => {
  ...
  useEffect(() => {
    ...
    else setselectedOL(null)
  }, [selectedOL]);

  useEffect(() => {
    if (errorupdate || errorAdd || errordelete || errorOL || emptyError) {
  ...
  }, [errorupdate, errorAdd, errordelete, errorOL, emptyError]);
  ...
  return (
    <>
      ...
      {showErrorTimer && errorOL &&
        <Error myError={"در دریافت لیست!"} />
      }

      {showErrorTimer && (errorupdate || errorAdd || errordelete) &&
