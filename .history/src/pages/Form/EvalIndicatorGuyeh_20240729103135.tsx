...
import EvalIndicatorGuyehUpdate from '../../hooks/EvalIndicatorGuyeh/EvalIndicatorGuyehUpdate';
...
const EvalIndicatorGuyeh: React.FC = () => {
  ...
  const { errorUpdateItem, updateEvalItem } = EvalIndicatorGuyehUpdate();
  ...
  const handleUpdateApi = async () => {
    ...
    if (newTitle !== "" && fields) {
      await updateEvalItem(selectedItem?.id,newTitle, location.state, fields);
  ...
  useEffect(() => {
   if(selectedItem) setNewTitle(selectedItem?.title)
  }, [selectedItem]);
  ...
  useEffect(() => {
    if (errorUpdateIndicator || errordelete || errorCreateIndicator) {
  ...
  }, [errorUpdateIndicator, errordelete, errorCreateIndicator]);
        ...
        <>
          <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none  text-sm">
            <div className="relative my-6 mx-auto w-1/3 items-center justify-center">
              <div className=" border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                <ModalTopic
                  modalTopicText="ویرایش گویه"
                  modalTopicCloseBtn={() => setupdateModal(false)}
                />
                ...
                <div className="flex items-center justify-end p-6 border-t border-solid border-zinc-200 rounded-b">
                  <PopupCloseBtn
                    PopupCloseBtnText="بستن"
                    PopupCloseBtnFunc={() => setupdateModal(false)}
                  />
                  <PopupAddBtn
                    PopupAddBtnText="افزودن گویه"
                    PopupAddBtnFunc={() => handleUpdateApi()}
                  />
                </div>
              </div>
            </div>
                ...
                <div className="relative p-6 flex-auto text-right">
                  {
                    selectedItemData.map((item, index) => (
                      <div className='flex row justify-between border-solid border-b border-zinc-300	m-3 p-3 bg-teal-50		'>
                                ...
                                <tr key={index} className="border-zinc-200">
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-neutral-200 border-zinc-200 text-center">
                                    ...
                                    <button
                                      ...
                                      type="button"
                                      onClick={() => {  handleShowInfo(field.id);setupdateModal(true); setselectedItem(field) }}
                                    ...
                                    <button
                                      ...
                                      type="button"
                                      onClick={() => { handleShowInfo(field.id); setdeleteModal(true); setselectedItem(field) }}
                                    >
                                      <img src={deleteIcon} />
                                    </button>
                                  </td>
