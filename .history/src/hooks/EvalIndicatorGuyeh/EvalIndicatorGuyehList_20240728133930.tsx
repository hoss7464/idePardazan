    ...
    const getEvalIndicatorGuyehListData = async (id: number) => {
        try {
            ...
            if (token) {
                ...
                headers.append('Authorization', 'Bearer ' + token); // اصلاح
            }

            const response = await fetch(
                https://mqtt-broker.ir/api/admin/evaluation-item/show/${id}, // اصلاح
                {
                    method: 'GET',
                    headers: headers,
                }
...
useEffect(() => {
    const fetchData = async () => {
        if (location.state) { // اطمینان از وجود state
            await getEvalIndicatorGuyehListData(location.state);
        }
    };
    fetchData();
}, [location.state]); // اضافه کردن location.state به وابستگی‌ها
